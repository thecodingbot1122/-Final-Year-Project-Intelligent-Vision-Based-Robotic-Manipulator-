...
void generateZigZagPath() {
  totalWaypoints = 0;
  int numLanes = 3; // Number of vertical columns/lanes
  
  for (int i = 0; i < numLanes; i++) {
    float fraction = (float)i / (numLanes - 1);
    
    // Calculate the bottom point of this lane (moving from Corner 0 to Corner 3)
    double bottomLat = G_Lat[0] + (G_Lat[3] - G_Lat[0]) * fraction;
    double bottomLon = G_Lon[0] + (G_Lon[3] - G_Lon[0]) * fraction;

    // Calculate the top point of this lane (moving from Corner 1 to Corner 2)
    double topLat = G_Lat[1] + (G_Lat[2] - G_Lat[1]) * fraction;
    double topLon = G_Lon[1] + (G_Lon[2] - G_Lon[1]) * fraction;

    // Zig-Zag Logic: Alternate driving up and driving down
    if (i % 2 == 0) {
      // Even lanes: Go from Bottom to Top
      waypointsLat[totalWaypoints] = bottomLat; waypointsLon[totalWaypoints] = bottomLon; totalWaypoints++;
      waypointsLat[totalWaypoints] = topLat;    waypointsLon[totalWaypoints] = topLon;    totalWaypoints++;
    } else {
      // Odd lanes: Go from Top to Bottom
      waypointsLat[totalWaypoints] = topLat;    waypointsLon[totalWaypoints] = topLon;    totalWaypoints++;
      waypointsLat[totalWaypoints] = bottomLat; waypointsLon[totalWaypoints] = bottomLon; totalWaypoints++;
    }
  }
  Serial.println(">>> Vertical ZigZag Path Generated! Total Waypoints: " + String(totalWaypoints));
}
